...
int choice, extra_charges[4] = { 0, 250, 370, 490 }, fares[4] = { 500, 600, 800, 700 }, seats, train_number, class_number, T_Fare, E, A, B, P, seat_limit, v;
char x, name[size], class_name[size], train_name[size], destinaton[size], Q1[10], Q2[10], Q3[10], Q4[10], Q5[10], Comments[100];
...
choice:
	...
	{
				system("cls");
				printf("<o> Feedback Forum:-\n");
				Feedback(); // User Defined Function
				break;
	}
	case 0: // Choice 5
	{
...
void Feedback(){ // When FEED BACK option is chosen from main menu
	char ch;
	printf("\n\t\t< Questions >\n\n");

	printf("Q1: How would you rate our Railway Staff?\n");
	printf("[ 1 = Bad | 2 = Fair | 3 = Good | 4 = Great | 5 = Excellent ]\n");
	feeds(v, Q1);
	printf("\nQ2: How would you rate our Railway Station and Platform\n");
	printf("[ 1 = Bad | 2 = Fair | 3 = Good | 4 = Great | 5 = Excellent ]\n");
	feeds(v, Q2);
	printf("\nQ3: How would you rate our Trains?\n");
	printf("[ 1 = Bad | 2 = Fair | 3 = Good | 4 = Great | 5 = Excellent ]\n");
	feeds(v, Q3);
	printf("\nQ4: How would you rate our Ticketing Process?\n");
	printf("[ 1 = Bad | 2 = Fair | 3 = Good | 4 = Great | 5 = Excellent ]\n");
	feeds(v, Q4);
	printf("\nQ5: How would you rate this NTC (National Train Companion)?\n");
	printf("[ 1 = Bad | 2 = Fair | 3 = Good | 4 = Great | 5 = Excellent ]\n");
	feeds(v, Q5);

	printf("\nNote: Kindly tell us below what lacks and what should be done to better the services.");
	printf("\n\n          (0 < Characters < 201 ) & ( Enter = Save )\n\n");

	printf("@ Comments: ");
	scanf(" %[^\n]s", Comments);

Tch:
	printf("\nAre you sure you want to submit this? ( Yes = y , No = n )=> ");
	scanf_s(" %c", &ch);
	if (ch == 'y')
	{
		SAVE_FORUM();
...
void feeds(int x, char Q[]){ // For saving Choices in Feedback Forum
def:
	fflush(stdin);
	x = 0;
	printf("> Choice: ");
...
void SAVE_FORUM(){ // For creating a txt file of Feedback Forum
	FILE *fs;
	fs = fopen("Feedback_forum.txt", "a");
	fprintf(fs, "  .----------------------------------------------------.\n");
	fprintf(fs, " |             F e e d b a c k __ f o r u m             |\n");
	fprintf(fs, "  '----------------------------------------------------'  \n");
	fprintf(fs, "     <o> Railway Staff       :    %s\n\n", Q1);
	fprintf(fs, "     <o> Railway Station     :    %s\n\n", Q2);
	fprintf(fs, "     <o> Trains              :    %s\n\n", Q3);
	fprintf(fs, "     <o> Ticketing Process   :    %s\n\n", Q4);
	fprintf(fs, "     <o> NTC Console         :    %s\n\n\n", Q5);
	fprintf(fs, "     <@> Comments: %s\n\n", Comments);
	fprintf(fs, " '------------------------------------------------------'\n\n");
	fclose(fs);
}
